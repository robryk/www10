...
public:
    ...
    business_logic()
    {
        rl::pthread_rwlock_init(&accounts_guard, 0, $);
    }

    ~business_logic()
    {
        rl::pthread_rwlock_destroy(&accounts_guard, $);
    }

    bool add_account(account_id_t acc_id, balance_t balance)
    {
        rl::pthread_rwlock_wrlock(&accounts_guard, $);
        if (accounts.find(acc_id) != accounts.end())
        {
            rl::pthread_rwlock_unlock(&accounts_guard, $);
            return false;
        }
    ...
    bool transfer_balance(account_id_t acc_id1, account_id_t acc_id2, balance_t amount)
    {
        if (acc_id1 == acc_id2)
            return true;
        rl::pthread_rwlock_rdlock(&accounts_guard, $);
        ...
        if (acc_id1 > acc_id2)
        {
            rl::pthread_mutex_lock(&acc1.mtx, $);
            rl::pthread_mutex_lock(&acc2.mtx, $);
        }
        else
        {
            rl::pthread_mutex_lock(&acc2.mtx, $);
        ...
        rl::pthread_mutex_unlock(&acc1.mtx, $);
        rl::pthread_mutex_unlock(&acc2.mtx, $);
        return true;
    }
...
private:
    ...
    {
        balance_t balance;
        rl::pthread_mutex_t mtx;

        account_info()
            : balance()
        {
            rl::pthread_mutex_init(&mtx, 0, $);
        }
